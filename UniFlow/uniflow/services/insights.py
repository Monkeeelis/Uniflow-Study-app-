...
# Fixed minute thresholds, so a colour always means the same amount of study
# time regardless of how busy the rest of the range was.
# 1: 0-10min, 2: 10-30min, 3: 30min-1hr, 4: 1hr-2hr, 5: 2hr+
        ...
        cells.append(
            {
                ...
                "minutes": minutes,
                "level": _level(minutes) if in_range else 0,
                "weekday": day.weekday(),
                "is_today": day == today,
                "in_range": in_range,
            }
...
def _weekly_minutes(data: dict[str, Any], today: datetime.date) -> int:
    ...
    return sum(
        minutes_map.get((start + datetime.timedelta(days=i)).isoformat(), 0)
        for i in range(7)
    )
...
def view(data: dict[str, Any]) -> dict[str, Any]:
    ...
    today = datetime.date.today()
    goal = data["onboarding"]["weekly_goal_minutes"]
    weekly_minutes = _weekly_minutes(data, today)
    weekly_percent = min(100, round(weekly_minutes / goal * 100)) if goal > 0 else 0
    ...
    return {
        ...
        "weekly_minutes_display": _hm_display(weekly_minutes),
        "weekly_goal_display": _hm_display(goal),
        "weekly_goal_percent": weekly_percent,
        "weekly_goal_met": weekly_minutes >= goal,
    }
